...
ava_1.test('health check responds correctly', async (t) => {
    const res = await server.get('/_ah/health');
    t.is(res.status, 200);
});
ava_1.test('renders basic script', async (t) => {
...
ava_1.test('renders script after page load event', async (t) => {
...
ava_1.test.failing('renders shadow DOM - no polyfill', async (t) => {
...
ava_1.test('script tags for JSON-LD are not stripped', async (t) => {
...
ava_1.test('screenshot accepts options', async (t) => {
...
ava_1.test('invalid url fails', async (t) => {
    const res = await server.get(`/render/abc`);
    t.is(res.status, 403);
});
ava_1.test('unknown url fails', async (t) => {
    const res = await server.get(`/render/http://unknown.blah.com`);
    t.is(res.status, 400);
});
ava_1.test('file url fails', async (t) => {
    const res = await server.get(`/render/file:///dev/fd/0`);
    t.is(res.status, 403);
});
ava_1.test('file url fails for screenshot', async (t) => {
    const res = await server.get(`/screenshot/file:///dev/fd/0`);
    t.is(res.status, 403);
});
ava_1.test.failing('explicit render event ends early', async (t) => {
...
//   t.is(res.status, 403);
// });
ava_1.test('unknown url fails safely on screenshot', async (t) => {
    const res = await server.get(`/render/http://unknown.blah.com`);
    t.is(res.status, 400);
});
